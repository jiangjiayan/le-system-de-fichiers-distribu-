...
from ..utils.merkle_tree import MerkleTree  # 确保导入路径正确
...
class Peer(MessageServer):
    ...
    def __init__(self, ):
        ...
        self.key_manager = KeyManager()#sigature
    ...
    async def connect(self, tracker_address):
        ...
        try:
            ...
            await self.send_signed_message(self._tracker_writer, {
                'type': MessageType.REQUEST_REGISTER,
                'address': self._server_address
            })
            message = await self.receive_and_verify_message(self._tracker_reader)
